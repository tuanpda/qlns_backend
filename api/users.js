  ...
  try {
    ...
    const result = await pool
      ...
      .input("createdAt", req.body.createdAt).query(`
                INSERT INTO users (cccd, sodienthoai, email, username, name, password, role, avatar, active, 
                  createdBy, createdAt) 
                VALUES (@cccd, @sodienthoai, @email, @username, @name, @password, @role, @avatar, @active,
                  @createdBy, @createdAt);
            `);
    const user = req.body;
    let token = jwt.sign({ user }, process.env.SECRET, { expiresIn: "1h" });
